...
export default function AdminVirtualQuotesPage() {
  return (
    <AdminOperationalTablePage
      ...
      actions={[
        { label: "En revision", path: (row) => `/api/admin/virtual-quotes/${encodeURIComponent(String(row.id))}/update-status`, body: () => ({ status: "pendiente_revision", message: "Admin marco la cotizacion en revision." }), tone: "secondary" },
        ...
        { label: "Convertida", path: (row) => `/api/admin/virtual-quotes/${encodeURIComponent(String(row.id))}/update-status`, body: () => ({ status: "convertida_a_reserva", message: "Cotizacion convertida en reserva." }), tone: "secondary" },
      ]}
    />
  );
}
